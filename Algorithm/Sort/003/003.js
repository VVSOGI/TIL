...
const result1 = quickSortWithLomuto([5, 4, 3, 2, 1]);
console.log(result1);
...
const swap2 = function (array, front, back) {
  const tmp = array[front];
  array[front] = array[back];
  array[back] = tmp;
};
...
const hoarePartition = function (array, start, end) {
  ...
  while (start <= end) {
    ...
    if (start <= end) {
      // 교차 되기 전이라면
      swap2(array, start, end); // pivotValue 보다 크거나 같은값과, 작거나 같은값을 swap 한다.
...
const result2 = quickSortWithHoare([5, 4, 7, 2, 1]);
console.log(result2);
